...
contains
  ...
  subroutine qag ( f, a, b, epsabs, epsrel, key, result, abserr, neval, ier, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
...
!!!!!
    ...
    call qage ( f, a, b, epsabs, epsrel, key, limit, result, abserr, neval, &
         ier, alist, blist, rlist, elist, iord, last, argv )
  ...
  subroutine qage ( f, a, b, epsabs, epsrel, key, limit, result, abserr, neval, &
    ...
    integer, intent(out) :: NEVAL
    integer, intent(out) :: IER
    real, intent(out) :: ALIST(LIMIT), BLIST(LIMIT)
    ...
    if ( keyf == 1 ) then
       call qk15 ( f, a, b, result, abserr, defabs, resabs , argv )
    else if ( keyf == 2 ) then
       call qk21 ( f, a, b, result, abserr, defabs, resabs , argv )
    else if ( keyf == 3 ) then
       call qk31 ( f, a, b, result, abserr, defabs, resabs , argv )
    else if ( keyf == 4 ) then
       call qk41 ( f, a, b, result, abserr, defabs, resabs , argv )
    else if ( keyf == 5 ) then
       call qk51 ( f, a, b, result, abserr, defabs, resabs , argv )
    else if ( keyf == 6 ) then
       call qk61 ( f, a, b, result, abserr, defabs, resabs , argv )
    end if
    ...
    do last = 2, limit
       ...
       if ( keyf == 1 ) then
          call qk15 ( f, a1, b1, area1, error1, resabs, defab1 , argv )
       else if ( keyf == 2 ) then
          call qk21 ( f, a1, b1, area1, error1, resabs, defab1 , argv )
       else if ( keyf == 3 ) then
          call qk31 ( f, a1, b1, area1, error1, resabs, defab1 , argv )
       else if ( keyf == 4 ) then
          call qk41 ( f, a1, b1, area1, error1, resabs, defab1 , argv )
       else if ( keyf == 5 ) then
          call qk51 ( f, a1, b1, area1, error1, resabs, defab1 , argv )
       else if ( keyf == 6 ) then
          call qk61 ( f, a1, b1, area1, error1, resabs, defab1 , argv )
       end if

       if ( keyf == 1 ) then
          call qk15 ( f, a2, b2, area2, error2, resabs, defab2 , argv )
       else if ( keyf == 2 ) then
          call qk21 ( f, a2, b2, area2, error2, resabs, defab2 , argv )
       else if ( keyf == 3 ) then
          call qk31 ( f, a2, b2, area2, error2, resabs, defab2 , argv )
       else if ( keyf == 4 ) then
          call qk41 ( f, a2, b2, area2, error2, resabs, defab2 , argv )
       else if ( keyf == 5 ) then
          call qk51 ( f, a2, b2, area2, error2, resabs, defab2 , argv )
       else if ( keyf == 6 ) then
          call qk61 ( f, a2, b2, area2, error2, resabs, defab2 , argv )
       end if
       !
  ...
  subroutine qagi ( f, bound, inf, epsabs, epsrel, result, abserr, neval, ier, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
...
!!!!!
    ...
    call qk15i ( f, boun, inf, 0.0e+00, 1.0e+00, result, abserr, defabs, resabs , argv )
    !
    !  Test on accuracy.
    !
    last = 1
    ...
    do last = 2, limit
       ...
       b2 = blist(maxerr)
       erlast = errmax
       call qk15i ( f, boun, inf, a1, b1, area1, error1, resabs, defab1 , argv )
       call qk15i ( f, boun, inf, a2, b2, area2, error2, resabs, defab2 , argv )
       !
...
130 continue
  ...
  subroutine qagp ( f, a, b, npts2, points, epsabs, epsrel, result, abserr, &
    ...
    real, intent(inout) :: POINTS(NPTS2)
    real, intent(in) :: EPSABS, EPSREL
    real, intent(out) :: RESULT
    real, intent(out) :: ABSERR
    integer, intent(out) :: NEVAL
...
!!!!!
    ...
    do i = 1, nint
       ...
       b1 = pts(i+1)
       call qk21 ( f, a1, b1, area1, error1, defabs, resa , argv )
    ...
    do last = npts2, limit
       ...
       b2 = blist(maxerr)
       erlast = errmax
       call qk21 ( f, a1, b1, area1, error1, resa, defab1 , argv )
       call qk21 ( f, a2, b2, area2, error2, resa, defab2 , argv )
       !
       !  Improve previous approximations to integral and error
       !  and test for accuracy.
       !
...
210 continue
  ...
  subroutine qags ( f, a, b, epsabs, epsrel, result, abserr, neval, ier, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
...
140 continue
  ...
  subroutine qawc ( f, a, b, c, epsabs, epsrel, result, abserr, neval, ier, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    call qawce ( f, a, b, c, epsabs, epsrel, limit, result, abserr, neval, ier, &
         alist, blist, rlist, elist, iord, last , argv )
  ...
  subroutine qawce ( f, a, b, c, epsabs, epsrel, limit, result, abserr, neval, &
    ...
    real, intent(out) :: RLIST(LIMIT)
    real, intent(out) :: ELIST(LIMIT)
    integer, intent(out), dimension(limit) :: iord
    integer, intent(out) :: last
    real, intent(in), dimension(:), optional :: argv
    ! PARAMS END
    !*****************************************************************************80
    ...
    krule = 1
    call qc25c ( f, aa, bb, c, result, abserr, krule, neval , argv )
    last = 1
    rlist(1) = result
    elist(1) = abserr
    iord(1) = 1
    ...
    do last = 2, limit
       ...
       call qc25c ( f, a1, b1, c, area1, error1, krule, nev , argv )
       neval = neval+nev

       call qc25c ( f, a2, b2, c, area2, error2, krule, nev , argv )
       neval = neval+nev
       !
       !  Improve previous approximations to integral and error
       !  and test for accuracy.
...
70  continue 
  ...
  subroutine qawf ( f, a, omega, integr, epsabs, result, abserr, neval, ier, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    call qawfe ( f, a, omega, integr, epsabs, limlst, limit, maxp1, &
         ...
         rlist, elist, iord, nnlog, chebmo , argv )
  ...
  subroutine qawfe ( f, a, omega, integr, epsabs, limlst, limit, maxp1, &
    ...
    integer, intent(out) :: NEVAL
    integer, intent(out) :: ier    
    real, intent(out) :: rslst(limlst)
    real, intent(out) :: erlst(limlst)
    integer, intent(out) :: ierlst(limlst)
    integer, intent(out) :: lst
    real, intent(out), dimension(limlst) :: alist, blist, rlist, elist
    integer, intent(out), dimension(limit) :: iord, nnlog
    real, intent(out) :: chebmo(maxp1,25)
    real, intent(in), dimension(:), optional :: argv
    ! PARAMS END
    !*****************************************************************************80
    !
    ...
    if ( omega == 0.0e+00 ) then
       ...
       if ( integr == 1 ) then
          call qagi ( f, 0.0e+00, 1, epsabs, 0.0e+00, result, abserr, neval, ier , argv )
       else
          result = 0.0E+00
          abserr = 0.0E+00
          neval = 0
...
80  continue
  ...
  subroutine qaws ( f, a, b, alfa, beta, integr, epsabs, epsrel, result, &
    ...
    call qawse ( f, a, b, alfa, beta, integr, epsabs, epsrel, limit, result, &
         abserr, neval, ier, alist, blist, rlist, elist, iord, last , argv )

    return
  end subroutine qaws
  subroutine qawse ( f, a, b, alfa, beta, integr, epsabs, epsrel, limit, &
       result, abserr, neval, ier, alist, blist, rlist, elist, iord, last, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    call qc25s ( f, a, b, a, centre, alfa, beta, ri, rj, rg, rh, area1, &
         error1, resas1, integr, nev , argv )

    neval = nev

    call qc25s ( f, a, b, centre, b, alfa, beta, ri, rj, rg, rh, area2, &
         error2, resas2, integr, nev , argv )
  ...
  subroutine qc25c ( f, a, b, c, result, abserr, krul, neval, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
...
!        function qwgtc ( x, c, p2, p3, p4, kp ) result(val)
!          real, intent(in) :: x, c, p2, p3, p4
...
!        end function qwgtc
    ...
    centr = 5.0e-01 * ( b + a )
    neval = 25
    fval(1) = 5.0e-01 * f(hlgth+centr, argv)
    fval(13) = f(centr, argv)
    fval(25) = 5.0e-01 * f(centr-hlgth, argv)

    do i = 2, 12
       u = hlgth * x(i-1)
       isym = 26 - i
       fval(i) = f(u+centr, argv)
       fval(isym) = f(centr-u, argv)
    end do
    !
  ...
  subroutine qc25o ( f, a, b, omega, integr, nrmom, maxp1, ksave, result, &
       abserr, neval, resabs, resasc, momcom, chebmo, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
...
140 continue
    ...
    !
    fval(1) = 5.0e-01 * f(centr+hlgth, argv)
    fval(13) = f(centr, argv)
    fval(25) = 5.0e-01 * f(centr-hlgth, argv)

    do i = 2, 12
       isym = 26-i
       fval(i) = f(hlgth*x(i-1)+centr, argv)
       fval(isym) = f(centr-hlgth*x(i-1), argv)
    end do
  ...
  subroutine qc25s ( f, a, b, bl, br, alfa, beta, ri, rj, rg, rh, result, &
       abserr, resasc, integr, neval, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    real, intent(in) :: A, B
    real, intent(in) :: BL, BR
    real, intent(in) :: ALFA, BETA
    real, intent(in) :: RI(25), RJ(25), RG(25), RH(25)
    real, intent(out) :: RESULT
    real, intent(out) :: ABSERR
    real, intent(out) :: RESASC
    integer, intent(in) :: INTEGR
...
10  continue
    ...
    centr = 5.0e-01*(br+bl)
    fix = b-centr
    fval(1) = 5.0e-01*f(hlgth+centr, argv)*(fix-hlgth)**beta
    fval(13) = f(centr, argv)*(fix**beta)
    fval(25) = 5.0e-01*f(centr-hlgth, argv)*(fix+hlgth)**beta

    do i = 2, 12
       u = hlgth*x(i-1)
       isym = 26-i
       fval(i) = f(u+centr, argv)*(fix-u)**beta
       fval(isym) = f(centr-u, argv)*(fix+u)**beta
    end do
...
140 continue
    ...
    centr = 5.0e-01*(br+bl)
    fix = centr-a
    fval(1) = 5.0e-01*f(hlgth+centr, argv)*(fix+hlgth)**alfa
    fval(13) = f(centr, argv)*(fix**alfa)
    fval(25) = 5.0e-01*f(centr-hlgth, argv)*(fix-hlgth)**alfa

    do i = 2, 12
       u = hlgth*x(i-1)
       isym = 26-i
       fval(i) = f(u+centr, argv)*(fix+u)**alfa
       fval(isym) = f(centr-u, argv)*(fix-u)**alfa
    end do
...
270 continue
  ...
  subroutine qcheb ( x, fval, cheb12, cheb24 )
    ! PARAMS BEG
    real, intent(in) :: X(11)
    real, intent(inout) :: FVAL(25)
    real, intent(out) :: CHEB12(13)
    real, intent(out) :: CHEB24(25)
    ! PARAMS END
    !*****************************************************************************80
    !
  ...
  subroutine qextr ( n, epstab, result, abserr, res3la, nres )
    ...
    real, intent(inout) :: EPSTAB(52)
    real, intent(out) :: RESULT
    real, intent(out) :: ABSERR
    real, intent(out) :: RES3LA(3)
    ...
    !
    !    intent(inout) :: real EPSTAB(52), the two lower diagonals of the triangular
...
30     continue
  ...
  subroutine qfour ( f, a, b, omega, integr, epsabs, epsrel, limit, icall, &
    ...
    call qc25o ( f, a, b, domega, integr, nrmom, maxp1, 0, result, abserr, &
         neval, defabs, resabs, momcom, chebmo , argv )
    !
    !  Test on accuracy.
    !
    dres = abs(result)
    ...
    do last = 2, limit
       ...
       call qc25o ( f, a1, b1, domega, integr, nrmom, maxp1, 0, area1, &
            error1, nev, resabs, defab1, momcom, chebmo , argv )

       neval = neval+nev

       call qc25o ( f, a2, b2, domega, integr, nrmom, maxp1, 1, area2, &
            error2, nev, resabs, defab2, momcom, chebmo , argv )
...
200 continue
  ...
  subroutine qk15 ( f, a, b, result, abserr, resabs, resasc, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    do j = 1, 3
       jtw = j*2
       absc = hlgth*xgk(jtw)
       fval1 = f(centr-absc, argv)
       fval2 = f(centr+absc, argv)
       fv1(jtw) = fval1
       fv2(jtw) = fval2
       fsum = fval1+fval2
       resg = resg+wg(j)*fsum
  ...
  subroutine qk15i ( f, boun, inf, a, b, result, abserr, resabs, resasc, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    fval1 = f(tabsc1, argv)
    if ( inf == 2 ) fval1 = fval1+f(-tabsc1, argv)
    ...
    do j = 1, 7
       ...
       if ( inf == 2 ) then
          fval1 = fval1+f(-tabsc1, argv)
          fval2 = fval2+f(-tabsc2, argv)
       end if
  ...
  subroutine qk15w ( f, w, p1, p2, p3, p4, kp, a, b, result, abserr, resabs, &
    ...
    interface
       ...
       function w(absc, P1, P2, P3, P4, kp) result(val)
         real, intent(in) :: absc, P1, P2, P3, P4
         integer, intent(in) :: kp
         real :: val
       end function w
    end interface
    ...
    !  and estimate the error.
    !
    fc = f(centr, argv)*w(centr,p1,p2,p3,p4,kp)
    resg = wg(4)*fc
    resk = wgk(8)*fc
    resabs = abs(resk)
    ...
    do j = 1, 3
       ...
       absc1 = centr-absc
       absc2 = centr+absc
       fval1 = f(absc1, argv)*w(absc1,p1,p2,p3,p4,kp)
       fval2 = f(absc2, argv)*w(absc2,p1,p2,p3,p4,kp)
       fv1(jtw) = fval1
       fv2(jtw) = fval2
       fsum = fval1+fval2
       resg = resg+wg(j)*fsum
  ...
  subroutine qk21 ( f, a, b, result, abserr, resabs, resasc, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
  ...
  subroutine qk31 ( f, a, b, result, abserr, resabs, resasc, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
  ...
  subroutine qk41 ( f, a, b, result, abserr, resabs, resasc, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
  ...
  subroutine qk51 ( f, a, b, result, abserr, resabs, resasc, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
  ...
  subroutine qk61 ( f, a, b, result, abserr, resabs, resasc, argv ) 
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
  ...
  subroutine qmomo ( alfa, beta, ri, rj, rg, rh, integr )
    ...
    real, intent(in) :: ALFA
    real, intent(in) :: BETA
    real, intent(out) :: rg(25)
    real, intent(out) :: rh(25)
    real, intent(out) :: ri(25)
    real, intent(out) :: rj(25)
    integer, intent(in) :: integr
    ! PARAMS END
    !*****************************************************************************80
    !
...
70  continue
  ...
  subroutine qng ( f, a, b, epsabs, epsrel, result, abserr, neval, ier, argv )
    ! PARAMS BEG
    interface
       function f(x, argv) result(val)
         real, intent(in) :: x
    ...
    do l = 1, 3
       ...
       if ( l == 1 ) then
          ...
          do k = 1, 11
             ipx = ipx + 1
             absc = hlgth * x3(k)
             fval = f(absc+centr, argv) + f(centr-absc, argv)
          ...
          do k = 1, 22
             absc = hlgth * x4(k)
             res87 = res87 + w87b(k) * ( f(absc+centr, argv) + f(centr-absc, argv) )
          end do
...
90  continue
  ...
  function qwgto ( x, omega, p2, p3, p4, integr ) result(val)
    real, intent(in) :: x, omega, p2, p3, p4
    integer, intent(in) :: integr
    real :: val
    !*****************************************************************************80
    !
  ...
  function qwgts ( x, a, b, alfa, beta, integr ) result(val)
    real, intent(in) :: x, a, b, alfa, beta
    integer, intent(in) :: integr
    real :: val
    !*****************************************************************************80
    !
    ...
    if ( integr == 1 ) then
       val = ( x - a )**alfa * ( b - x )**beta
    else if ( integr == 2 ) then
       val = ( x - a )**alfa * ( b - x )**beta * log ( x - a )
    else if ( integr == 3 ) then
       val = ( x - a )**alfa * ( b - x )**beta * log ( b - x )
    else if ( integr == 4 ) then
       val = ( x - a )**alfa * ( b - x )**beta * log ( x - a ) * log ( b - x )
    end if
